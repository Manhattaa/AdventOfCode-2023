...
namespace Day2
{
    ...
    {
        internal static string _redName = "red";
        internal static string _blueName = "blue";
        internal static string _greenName = "green";

        static void Main()
        {
            var red_cubes_count = 12;
            var green_cubes_count = 13;
            var blue_cubes_count = 14;

            var games = new List<Game>();

            var gamesLines = _input.Split(Environment.NewLine);
            foreach (var gameLine in gamesLines)
            {
                games.Add(ParseGame(gameLine));
            }
            ...
            {
                if (game.Sets.Any(set => set.Red > red_cubes_count || set.Blue > blue_cubes_count || set.Green > green_cubes_count))
                {
                    continue;
                }
            ...
            var validGamesIDsSum = validGames.Sum(game => game.Id);

            Console.WriteLine("Valid games IDs sum: " + validGamesIDsSum);
            Console.WriteLine("=== PART 2 ===");

            var sumOfPowers = 0;
            foreach (var game in games)
            {
                var minGreen = GetMinimumOfRequiredCubesFromGame(game, set => set.Green);
                var minBlue = GetMinimumOfRequiredCubesFromGame(game, set => set.Blue);
                var minRed = GetMinimumOfRequiredCubesFromGame(game, set => set.Red);
                ...
                var setPower = minGreen * minBlue * minRed;
                sumOfPowers += setPower;
            }

            Console.WriteLine("Sum of the power of min. sets: " + sumOfPowers);
            Console.ReadLine();
        }

        private static Game ParseGame(string input)
        {
            var gamePart = input.Split(":");
            var gameID = int.Parse(gamePart[0].Split("Game ")[1]);
            var game = new Game()
            {
                Id = gameID
            };

            var setParts = gamePart[1].Split(";");
            foreach (var set in setParts)
            {
                game.AddSet(ParseSet(set));
            }
        ...
        {
            var set = new Set();
            var cubesParts = input.Split(",");
            foreach (var cubePart in cubesParts)
            {
                var trimmed = cubePart.Trim();
                if (trimmed.Contains(_greenName, StringComparison.OrdinalIgnoreCase))
                {
                    set.Green = int.Parse(trimmed.Split(" ")[0]);
                }
                else if (trimmed.Contains(_redName, StringComparison.OrdinalIgnoreCase))
                {
                    set.Red = int.Parse(trimmed.Split(" ")[0]);
                }
                else if (trimmed.Contains(_blueName, StringComparison.OrdinalIgnoreCase))
                {
                    set.Blue = int.Parse(trimmed.Split(" ")[0]);
                }
            }
        ...
        private static int GetMinimumOfRequiredCubesFromGame(Game game, Func<Set, int> setSelector)
        {
            var validSets =
              game
              .Sets
              .Where(set => setSelector(set) != 0)
              .ToList();
            ...
            // Max is needed because we need the minimum of the cubes to play the game => minimum to play = maximum of the sets :)
            return validSets.Max(set => setSelector(set));
        }

        private class Game
        {
            public required int Id { get; init; }
            public ImmutableList<Set> Sets => _sets.ToImmutableList();
